...
## Updated by V. YAU, Jun 2023 - DIAS GEOPHYSICAL LTD.
...
def read_node_library(path, node, library, num, sessionID, forceloc=[]):
    messages = []
    ...
    for datFile in datNode:  # loop on DAT files in folder
        ...
        if test == 1:
            ...
            # to force a location to a node
            if len(forceloc) != 0: 
                gpsLocations = forceloc # Getting GPS location from input 
            else:
...
# def read_node_library_OLD(path, node, library, num):
#     messages = []
#     print(multiprocessing.current_process().name + " taking care of node " + node)
#     messages.append(multiprocessing.current_process().name + " taking care of node " + node + '\n')
#     records_node = []
